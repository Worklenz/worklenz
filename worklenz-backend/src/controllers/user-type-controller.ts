...
import { appSumoMigrationService } from "../shared/private-extensions";
import { CustomPlanMappingService } from "../services/custom-plan-mapping-service";

import { UserType, PlanTier } from "../interfaces/plan-recommendation";
...
export default class UserTypeController extends WorklenzControllerBase {
  ...
  public static async getUserType(
    ...
    try {
      ...
      const userTypeInfo = {
        ...
        limits: await this.getCurrentLimits(
          userAnalytics.userAnalytics.userType,
          organizationId
        ),
      ...
      return res
        ...
        .send(
          new ServerResponse(
            false,
            null,
            "Failed to retrieve user type information"
          )
        );
    }
  }
   ...
   */
  @HandleExceptions()
  public static async getLegacyPlan(
    req: IWorkLenzRequest,
    res: IWorkLenzResponse
  ): Promise<IWorkLenzResponse> {
    const organizationId = req.user?.organization_id;
    ...
    try {
      const [customPlanDetails, appSumoStatus, userAnalytics] =
        await Promise.all([
      ...
      if (appSumoStatus && appSumoStatus.isAppSumoUser) {
        const appSumoRecommendations =
          await appSumoMigrationService.getAppSumoRecommendations(
            organizationId
          );
        ...
        legacyPlan = {
          ...
          specialOffers: {
            ...
            postDiscountOptions:
              appSumoRecommendations?.postDiscountOptions || null,
          },
        };
      }
      ...
      if (userAnalytics.userAnalytics.userType === UserType.TRIAL) {
        ...
        legacyPlan = {
          ...
          details: trialInfo,
          migrationOptions:
            userAnalytics.userAnalytics.migrationEligibility.eligiblePlans,
      ...
      const response = {
        ...
        availableTransitions: await this.getAvailableTransitions(
          userAnalytics.userAnalytics.userType,
          organizationId
        ),
      };
      ...
      return res
        ...
        .send(
          new ServerResponse(
            false,
            null,
            "Failed to retrieve legacy plan information"
          )
        );
    }
  }
   ...
   */
  @HandleExceptions()
  public static async updateUserType(
    req: IWorkLenzRequest,
    res: IWorkLenzResponse
  ): Promise<IWorkLenzResponse> {
    const organizationId = req.user?.organization_id;
      ...
      return res
        ...
        .send(
          new ServerResponse(
            false,
            null,
            "Organization ID and new user type are required"
          )
        );
    }
    ...
    try {
      ...
      if (!transitionValidation.isValid) {
        return res
          .status(400)
          .send(new ServerResponse(false, null, transitionValidation.reason));
      }
      ...
      if (!changeResult.success) {
        return res
          .status(400)
          .send(new ServerResponse(false, null, changeResult.message));
      }
      ...
      return res.status(200).send(
        new ServerResponse(
          true,
          {
            ...
            updatedCapabilities: await this.getUserCapabilities(
              newUserType as UserType,
              organizationId
            ),
            newMigrationOptions:
              updatedAnalytics.userAnalytics.migrationEligibility,
          },
      ...
      return res
        .status(500)
        .send(new ServerResponse(false, null, "Failed to update user type"));
    }
  }
   ...
   */
  @HandleExceptions()
  public static async getUserTypeHistory(
    req: IWorkLenzRequest,
    res: IWorkLenzResponse
  ): Promise<IWorkLenzResponse> {
    const organizationId = req.user?.organization_id;
    ...
    try {
      const history = await this.getUserTypeTransitionHistory(
        organizationId,
      ...
      return res
        ...
        .send(
          new ServerResponse(
            false,
            null,
            "Failed to retrieve user type history"
          )
        );
    }
  }
  ...
  public static async checkEligibility(
      ...
      return res
        ...
        .send(
          new ServerResponse(
            false,
            null,
            "Organization ID and actions array are required"
          )
        );
    }
    ...
    try {
      ...
      for (const action of actions) {
        const eligibility = await this.checkActionEligibility(
          currentUserType,
          action,
          organizationId
        );
      ...
      return res.status(200).send(
        new ServerResponse(true, {
          ...
          overallEligibilityScore:
            this.calculateOverallEligibilityScore(eligibilityResults),
      ...
      return res
        .status(500)
        .send(new ServerResponse(false, null, "Failed to check eligibility"));
    }
  }
   ...
   * Helper methods
   */
  private static async getUserCapabilities(
    userType: UserType,
    organizationId: string
  ): Promise<any> {
    const baseCapabilities = this.getBaseCapabilities(userType);
    const currentUsage = await UserAnalyticsService.analyzeUsagePatterns(
      organizationId
    );
    ...
    return {
      ...
      utilizationRates: {
        users:
          baseCapabilities.limits.maxUsers > 0
            ? (currentUsage.totalUsers / baseCapabilities.limits.maxUsers) * 100
            : 0,
        storage:
          baseCapabilities.limits.storageLimit > 0
            ? (currentUsage.storageUsed /
                baseCapabilities.limits.storageLimit) *
              100
            : 0,
      },
    };
  ...
  private static async getCurrentLimits(
    userType: UserType,
    organizationId: string
  ): Promise<any> {
    ...
    return {
      users: {
        ...
        remaining:
          capabilities.limits.maxUsers > 0
            ? capabilities.limits.maxUsers - currentUsage.totalUsers
            : -1,
      },
      projects: {
        ...
        remaining:
          capabilities.limits.maxProjects > 0
            ? capabilities.limits.maxProjects - currentUsage.totalProjects
            : -1,
      },
      storage: {
        ...
        remaining:
          capabilities.limits.storageLimit > 0
            ? capabilities.limits.storageLimit - currentUsage.storageUsed
            : -1,
      },
    };
  }

  private static async getSpecialStatuses(
    organizationId: string
  ): Promise<any> {
  ...
  private static async getTrialInformation(
    organizationId: string
  ): Promise<any> {
    const query = `
  ...
  private static async getFreePlanInformation(
    organizationId: string
  ): Promise<any> {
    const query = `
  ...
  private static async getMigrationHistory(
    organizationId: string
  ): Promise<any[]> {
    const query = `
      SELECT 
  ...
  private static async getAvailableTransitions(
    ...
    switch (currentUserType) {
      case UserType.FREE:
        transitions.push(
          ...
          {
            to: UserType.ACTIVE_SUBSCRIBER,
            method: "direct_upgrade",
      ...
      case UserType.CUSTOM_PLAN:
        transitions.push({
          to: UserType.ACTIVE_SUBSCRIBER,
          method: "migration",
          requirements: ["plan_selection", "migration_consent"],
        });
        break;
      ...
      case UserType.APPSUMO:
        transitions.push({
          to: UserType.ACTIVE_SUBSCRIBER,
          method: "appsumo_migration",
          requirements: ["business_plan_selection", "migration_consent"],
        });
        break;
    }
  ...
  private static async recordUserTypeChange(
    params: any
  ): Promise<{ success: boolean; changeId?: string; message: string }> {
    try {
      const query = `
  ...
  private static async getUserTypeTransitionHistory(
    organizationId: string,
    limit: number
  ): Promise<any[]> {
    const query = `
  ...
  private static async getUserTypeAnalytics(
    organizationId: string
  ): Promise<any> {
  ...
  private static async getCurrentTypeStatus(
    organizationId: string
  ): Promise<any> {
    const userAnalytics =
      await PlanRecommendationService.generateRecommendations(organizationId);
    const userProgression =
      await UserAnalyticsService.analyzeUserTypeProgression(organizationId);
  ...
  private static async checkActionEligibility(
    ...
    const actionRequirements: Record<
      string,
      { minType: UserType; requiredPlans: PlanTier[] }
    > = {
      add_unlimited_users: {
        minType: UserType.ACTIVE_SUBSCRIBER,
        requiredPlans: [
          PlanTier.PRO_LARGE,
  ...
  private static calculateOverallEligibilityScore(
  ...
  private static getRecommendedActions(
    ...
    if (ineligibleActions.length > 0) {
      const mostCommonRequirement =
        this.findMostCommonRequirement(ineligibleActions);
      recommendations.push(
        `Consider upgrading to ${mostCommonRequirement} to unlock more features`
      );
    }

    if (currentUserType === UserType.TRIAL) {
      recommendations.push(
        "Convert trial to paid subscription to retain access"
      );
    }

    if (currentUserType === UserType.FREE) {
      recommendations.push(
        "Upgrade to Pro or Business plan for advanced features"
      );
    }
  ...
  private static findMostCommonRequirement(ineligibleActions: any[]): string {
    const requirements = ineligibleActions
      .map((a) => a.requiredUserType)
      .filter((r) => r);
