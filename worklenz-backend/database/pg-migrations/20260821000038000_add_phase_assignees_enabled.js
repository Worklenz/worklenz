'use strict';
// Converted from database/migrations/20260616000000-add-phase-assignees-enabled.sql
...
exports.up = async (pgm) => {
  pgm.sql(`
-- Migration: Add phase_assignees_enabled feature flag to projects
-- Description: Adds a boolean toggle to enable phase-level assignee enforcement per project.
--              When enabled, each pipeline phase can have a dedicated assignee who is the only
--              one allowed to mark tasks complete in that phase (feature gate for Task 1).
-- Date: 2026-06-16

-- 1. Add project-level toggle column
ALTER TABLE projects
  ADD COLUMN IF NOT EXISTS phase_assignees_enabled BOOLEAN DEFAULT FALSE;

COMMENT ON COLUMN projects.phase_assignees_enabled IS
  'When TRUE, each project phase can have a dedicated assignee. Tasks can only be advanced by their phase assignee (or an admin override).';

-- 2. Update create_project to include phase_assignees_enabled
...
BEGIN
    ...
    INSERT INTO projects (name, key, notes, color_code, team_id, client_id, owner_id, status_id, health_id, priority_id, start_date,
                          ...
                          restrict_task_creation, phase_assignees_enabled)
...
-- 3. Update update_project to include phase_assignees_enabled
