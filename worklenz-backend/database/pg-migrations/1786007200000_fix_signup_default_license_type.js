'use strict';
// Fixes register_user, register_google_user, and register_apple_user so new
// signups default to the FREE plan instead of SELF_HOSTED. SELF_HOSTED
// currently grants automatic business-plan access (see
// worklenz-backend/src/ee/business.ts and
// worklenz-frontend/src/utils/subscription-utils.ts), so every new SaaS
// signup was getting free business access. This migration only changes which
// sys_license_types row gets assigned at signup — no other logic changes.
//
// Function bodies are copied verbatim from their current definitions
// (register_user: 1758326401000_add_team_lead_role_migration.js,
// register_google_user: 1767571203000_fix_google_oauth_invite_setup_completed.js,
// register_apple_user: 1763049601000_add_register_apple_user_function.js)
// with only the license_type_id lookup key changed.
...
BEGIN
    ...
    VALUES (_user_id, TRIM((_body ->> 'team_name')::TEXT), NULL, NULL, TRUE, CURRENT_DATE + INTERVAL '9999 days',
            'active', (SELECT id FROM sys_license_types WHERE key = 'FREE'))
...
BEGIN
    ...
    VALUES (
        ...
        'active',
        (SELECT id FROM sys_license_types WHERE key = 'FREE')
    )
    RETURNING id INTO _organization_id;
