...
exports.down = pgm => {
  // Revert to a simpler version of the function (basic implementation)
  // Note: This is a simplified rollback - you may need to adjust based on your previous function version
  pgm.sql(`
    CREATE OR REPLACE FUNCTION get_task_complete_ratio(_task_id uuid) RETURNS json
        LANGUAGE plpgsql
    AS
    ...
    BEGIN
        ...
        FROM tasks 
        WHERE id = _task_id
        INTO _is_manual, _manual_value, _task_complete;
        ...
        -- Use manual progress value if set
        ...
        IF _sub_tasks_count > 0 THEN
            ...
            _total_tasks = _sub_tasks_count + 1; -- +1 for the parent task
        ELSE
            _total_completed = _parent_task_done;
            _total_tasks = 1;
        END IF;
