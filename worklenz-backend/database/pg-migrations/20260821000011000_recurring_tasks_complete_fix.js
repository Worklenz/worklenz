'use strict';
// Converted from database/migrations/20260210000000-recurring-tasks-complete-fix.sql
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'task_recurring_schedules_timezone_id_fk') THEN
    ALTER TABLE task_recurring_schedules ADD CONSTRAINT task_recurring_schedules_timezone_id_fk FOREIGN KEY (timezone_id) REFERENCES timezones(id)
    ON DELETE SET NULL;
  END IF;
END $$;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'task_recurring_schedules_created_by_fk') THEN
    ALTER TABLE task_recurring_schedules ADD CONSTRAINT task_recurring_schedules_created_by_fk FOREIGN KEY (created_by) REFERENCES users(id)
    ON DELETE SET NULL;
  END IF;
END $$;
