...
CREATE INDEX IF NOT EXISTS idx_task_updates_user_project_type_sent
ON task_updates(user_id, project_id, type, is_sent)
WHERE is_sent = FALSE;

CREATE INDEX IF NOT EXISTS idx_task_updates_sent_status
ON task_updates(is_sent, type)
WHERE is_sent = FALSE;

CREATE INDEX IF NOT EXISTS idx_task_updates_cleanup
