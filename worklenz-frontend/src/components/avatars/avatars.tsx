...
const Avatars: React.FC<AvatarsProps> = React.memo(
  ({ members, maxCount, allowClickThrough = false }) => {
    const displayMembers = maxCount ? members.slice(0, maxCount) : members;
    const hiddenMembers = maxCount ? members.slice(maxCount) : [];
    const hasOverflow = maxCount && members.length > maxCount;
