...
const CustomMentionsInput = ({
  ...
  const updateDropdownPos = () => {
    ...
    const showAbove = spaceAbove >= dropdownMaxH + 8;
    setDropdownPos({
      top: showAbove ? rect.top - dropdownMaxH - 4 : rect.bottom + 4,
      left: rect.left,
      width: rect.width,
    });
  };
  ...
  // Close dropdown on outside click — works with portal because dropdownRef still points to the DOM node
  useEffect(() => {
  ...
  return (
    <div className="custom-mentions-wrapper" style={{ position: 'relative' }}>
      ...
      {isDropdownOpen && filteredOptions.length > 0 && dropdownPos &&
        createPortal(
          <div
            ref={dropdownRef}
          ...
          >
            {filteredOptions.map((option, index) => (
              <div
                ...
                onMouseDown={e => {
                  // Use mousedown so the editable doesn't lose focus before we handle the click
                  e.preventDefault();
                  selectOption(option);
                }}
