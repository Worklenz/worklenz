...
const HomeLeftSidebar: React.FC<HomeLeftSidebarProps> = ({ activeView, onViewChange }) => {
  ...
  const { resolved, pin, unpin, isPinned, reorder, toggleCollapsed } = useNavPreferences('home');
  ...
  const trialDaysLeft = React.useMemo(() => {
    ...
    const expiry =
      currentSession?.valid_till_date || (currentSession as any)?.trial_expire_date;
  ...
  return (
    <div
      ...
      style={{
        ...
        flexDirection: 'column',
        background: bgColor,
        /* fixed height = viewport minus top navbar (52px) */
    ...
    >
      <NavRail
        ...
        isDark={isDark}
        onSelect={key => onViewChange(key as HomeView)}
        isPinned={isPinned}
        onPin={pin}
        onUnpin={unpin}
        onReorder={reorder}
      ...
      <div style={{
        ...
        {isTrial && trialDaysLeft !== null && (
          <Tooltip title={`${trialDaysLeft} days left in your trial`} placement="right">
            <button
              onClick={() => dispatch(toggleUpgradeModal())}
              style={{
                display: 'flex', flexDirection: 'column',
                alignItems: 'center', justifyContent: 'center',
                width: 48, padding: '4px 2px',
                background: 'transparent', cursor: 'pointer', border: 'none', gap: 0,
              }}
            >
        ...
        {/* Upgrade button */}
        {isTrial && (
          <Tooltip title="Upgrade to Pro" placement="right">
            <button
              onClick={() => dispatch(toggleUpgradeModal())}
              style={{
                display: 'flex', flexDirection: 'column',
                alignItems: 'center', justifyContent: 'center',
                gap: 2, width: '100%', padding: '8px 4px',
                borderRadius: 7, cursor: 'pointer', border: 'none',
                background: 'linear-gradient(135deg,#ff4d4f,#ff7875)',
                color: '#fff', fontSize: 9, fontWeight: 700, letterSpacing: .2,
              }}
              onMouseEnter={e => {
          ...
          content={
            <div style={{ width: 200, padding: '4px 0' }}>
              {SETTINGS_QUICK_LINKS.map(link => (
                <div
                  key={link.label}
                  onClick={() => { setSettingsOpen(false); navigate(link.to); }}
                  style={{
                    display: 'flex', alignItems: 'center', gap: 10,
                    padding: '8px 12px', cursor: 'pointer', fontSize: 13,
                    color: isDark ? 'rgba(255,255,255,.85)' : 'rgba(0,0,0,.85)',
                  }}
                  onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.background = isDark ? 'rgba(255,255,255,.06)' : 'rgba(0,0,0,.04)'; }}
              ...
              >
                ...
                <span style={{
                  ...
                  background: isDark ? '#1677ff' : 'rgba(0,0,0,.08)',
        ...
        >
          <Tooltip title="Settings" placement="right">
            <button
              style={{
                ...BOTTOM_BTN,
                color: settingsOpen ? '#1677ff' : iconColor,
                background: settingsOpen ? (isDark ? 'rgba(22,119,255,.18)' : 'rgba(22,119,255,.08)') : 'transparent',
              }}
              onMouseEnter={e => {
                if (!settingsOpen) {
        ...
        <Tooltip title="Get Mobile App" placement="right">
          <button
            onClick={() => setMobileModalOpen(true)}
            style={{
              ...BOTTOM_BTN,
              color: mobileModalOpen ? '#1677ff' : iconColor,
              background: mobileModalOpen ? (isDark ? 'rgba(22,119,255,.18)' : 'rgba(22,119,255,.08)') : 'transparent',
            }}
            onMouseEnter={e => {
              if (!mobileModalOpen) {
                const el = e.currentTarget as HTMLButtonElement;
          ...
          content={
            <div style={{ width: 200, padding: '4px 0' }}>
              {HELP_QUICK_LINKS.map(link => (
                <div
                  key={link.label}
                  onClick={() => { setHelpOpen(false); link.onClick(); }}
                  style={{
                    display: 'flex', alignItems: 'center', gap: 10,
                    padding: '8px 12px', cursor: 'pointer', fontSize: 13,
                    color: isDark ? 'rgba(255,255,255,.85)' : 'rgba(0,0,0,.85)',
                  ...
                  onMouseEnter={e => {
                    ...
                    el.style.background = isDark ? 'rgba(22,119,255,.18)' : 'rgba(22,119,255,.08)';
                    el.style.color = '#1677ff';
                  }}
                  onMouseLeave={e => {
                    const el = e.currentTarget as HTMLDivElement;
        ...
        >
          <Tooltip title="Help & Support" placement="right">
            <button
              style={{
                ...BOTTOM_BTN,
                color: helpOpen ? '#1677ff' : iconColor,
                background: helpOpen ? (isDark ? 'rgba(22,119,255,.18)' : 'rgba(22,119,255,.08)') : 'transparent',
              }}
              onMouseEnter={e => {
                if (!helpOpen) {
                  const el = e.currentTarget as HTMLButtonElement;
