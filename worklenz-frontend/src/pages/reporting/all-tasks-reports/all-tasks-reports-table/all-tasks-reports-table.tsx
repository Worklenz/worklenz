  ...
  const allColumns: TableColumnsType<IProjectTask> = useMemo(() => [
    {
      ...
      render: (_, record) => (
        ...
        >
          {Number(record.sub_tasks_count) > 0 && <DoubleRightOutlined style={{ fontSize: 10, flexShrink: 0 }} />}
    ...
    },
    {
      ...
      sorter: true,
      render: (value) => (
        <Typography.Text type={value && Number(value) > 0 ? 'danger' : undefined} style={{ fontSize: 12 }}>
          {value || '-'}
        </Typography.Text>
      ),
    },
      ...
      render: (_, record) => {
        ...
        return (
          <Flex gap={2} wrap="nowrap">
            ...
            {labels.length > 1 && (
              <Tooltip title={labels.slice(1).map(l => l.name).join(', ')}>
    ...
    },
    {
      ...
      sorter: true,
      render: (value) => (
        <Typography.Text style={{ fontSize: 12 }}>{value !== undefined && value !== null ? `${value}%` : '-'}</Typography.Text>
      ),
    },
    {
      key: 'subtasksCount',
      ...
      sorter: true,
      render: (value) => (
        <Typography.Text style={{ fontSize: 12 }}>{value || '0'}</Typography.Text>
      ),
    },
  ], [t]);
