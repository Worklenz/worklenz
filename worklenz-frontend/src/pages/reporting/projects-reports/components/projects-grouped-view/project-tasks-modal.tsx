...
const ProjectTasksModal = ({ open, project, onClose }: ProjectTasksModalProps) => {
  ...
  const [taskStats, setTaskStats] = useState<TaskStats>({ total: 0, completed: 0, inProgress: 0, overdue: 0 });
  ...
  }, [project?.id, currentPage, pageSize, searchQuery, statusFilter, priorityFilter, assigneeFilter, sortField, sortOrder]);
  ...
  const handleSearchChange = useCallback((value: string) => {
    ...
    searchTimeoutRef.current = setTimeout(() => {
      setCurrentPage(1);
      fetchTasks(1, pageSize, value, statusFilter, priorityFilter, assigneeFilter, sortField, sortOrder);
    }, 500);
  ...
  const handleStatusChange = useCallback((value: StatusFilter) => {
    ...
    setCurrentPage(1);
    fetchTasks(1, pageSize, searchQuery, value, priorityFilter, assigneeFilter, sortField, sortOrder);
  ...
  const handlePriorityChange = useCallback((value: PriorityFilter) => {
    ...
    setCurrentPage(1);
    fetchTasks(1, pageSize, searchQuery, statusFilter, value, assigneeFilter, sortField, sortOrder);
  }, [fetchTasks, pageSize, searchQuery, statusFilter, assigneeFilter, sortField, sortOrder]);

  const handleAssigneeChange = useCallback((value: string) => {
    setAssigneeFilter(value);
    setCurrentPage(1);
    fetchTasks(1, pageSize, searchQuery, statusFilter, priorityFilter, value, sortField, sortOrder);
  }, [fetchTasks, pageSize, searchQuery, statusFilter, priorityFilter, sortField, sortOrder]);

  const handleTableChange = useCallback((pagination: TablePaginationConfig, _filters: any, sorter: SorterResult<IProjectTask> | SorterResult<IProjectTask>[]) => {
    ...
    fetchTasks(newPage, newPageSize, searchQuery, statusFilter, priorityFilter, assigneeFilter, newSortField, newSortOrder);
  ...
  const columns = useMemo(() => [
    {
      ...
      ellipsis: true,
      sorter: true,
      sortOrder: (sortField === 'name' ? (sortOrder === 'asc' ? 'ascend' : 'descend') : undefined) as SortOrder,
    ...
    },
    {
      ...
      width: 110,
      sorter: true,
      sortOrder: (sortField === 'end_date' ? (sortOrder === 'asc' ? 'ascend' : 'descend') : undefined) as SortOrder,
  ...
  ), [searchQuery, statusFilter, priorityFilter, assigneeFilter, projectMembers, tasks.length, total, t, handleSearchChange, handleStatusChange, handlePriorityChange, handleAssigneeChange]);
      ...
      <Spin spinning={loading}>
        ...
        <Table
          ...
          pagination={{
            ...
            showTotal: (totalCount, range) => `${range[0]}-${range[1]} ${t('ofText')} ${totalCount}`,
          }}
