...
import {IMemberTaskListGroup} from "@interfaces/schedular";
import {ScheduleMemberTasksHashmapService} from "./schedule-member-tasks-hashmap-service.service";

@Injectable({
  providedIn: 'root'
})
export class ScheduleMemberTasksService {
private readonly membersSbj$ = new Subject<void>();
  ...
  public groups: IMemberTaskListGroup[] = [];
  ...
  private get _currentGroup(): IGroupByOption {
    const key = localStorage.getItem("worklenz.schedule.group_by");
    if (key) {
  ...
  private set _currentGroup(option) {
    localStorage.setItem("worklenz.schedule.group_by", option.value);
  }
