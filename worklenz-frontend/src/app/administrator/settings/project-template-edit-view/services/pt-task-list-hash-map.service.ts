...
import {IPTTask, IPTTaskListGroup} from "../interfaces";

@Injectable({
  providedIn: 'root'
})
export class PtTaskListHashMapService {
  private readonly _selectSbj$ = new Subject<IPTTask>();
  private readonly _deselectSbj$ = new Subject<IPTTask>();
  ...
  /** Map<task id, Task> */
  private readonly _selectedTasksMap = new Map<string, IPTTask>();
  /** Map<task id, Task> */
  private readonly _allTasksMap = new Map<string, IPTTask>();
  /** Map<task id, [SubTasks] */
  public readonly _subTasksMap = new Map<string, IPTTask[]>();
  ...
  public registerGroup(group: IPTTaskListGroup) {
    for (const task of group.tasks) {
      this.add(group.id, task);
    }
  }

  public add(groupId: string, task: IPTTask) {
    if (!task.id) return;
  ...
  public addGroupTask(groupId: string, task: IPTTask) {
    if (!task.id) return;
    this._taskGroupIdsMap.set(task.id, groupId);
  }
  ...
  private updateSubtasksMap(parentTaskId: string, task: IPTTask, selected?: boolean) {
    ...
    public selectTask(task: IPTTask) {
  ...
  public deselectTask(task: IPTTask) {
