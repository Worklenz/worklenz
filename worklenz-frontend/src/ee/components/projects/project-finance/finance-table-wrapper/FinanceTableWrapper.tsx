...
import { IProjectFinanceGroup, IProjectFinanceTask } from '@/ee/types/project/project-finance.types';
...
const FinanceTableWrapper: React.FC<FinanceTableWrapperProps> = ({ activeTablesList, loading }) => {
  ...
  const customColumnHeaderStyles = (key: FinanceTableColumnKeys) =>
    `px-2 ${key === FinanceTableColumnKeys.TASK ? 'text-left sticky left-0 z-10' : key === FinanceTableColumnKeys.MEMBERS ? `text-right sticky left-[240px] z-10 ${isScrolling ? 'after:content after:absolute after:top-0 after:-right-1 after:-z-10  after:h-[68px] after:w-1.5 after:bg-transparent after:bg-gradient-to-r after:from-[rgba(0,0,0,0.12)] after:to-transparent' : ''}` : 'text-right'} ${themeMode === 'dark' ? 'bg-[#1d1d1d] border-[#303030]' : 'bg-[#fafafa]'}`;

  const customColumnStyles = (key: FinanceTableColumnKeys) =>
    `px-2 ${key === FinanceTableColumnKeys.TASK ? `text-left sticky left-0 z-10 ${isScrolling ? 'after:content after:absolute after:top-0 after:-right-1 after:-z-10 after:h-[56px] after:w-1.5 after:bg-transparent after:bg-gradient-to-r after:from-[rgba(0,0,0,0.12)] after:to-transparent' : ''}` : key === FinanceTableColumnKeys.MEMBERS ? `text-right sticky left-[240px] z-10 ${isScrolling ? 'after:content after:absolute after:top-0 after:-right-1 after:-z-10 after:h-[56px] after:w-1.5 after:bg-transparent after:bg-gradient-to-r after:from-[rgba(0,0,0,0.12)] after:to-transparent' : ''}` : 'text-right'} ${themeMode === 'dark' ? 'bg-[#141414]' : 'bg-[#fbfbfb]'}`;
