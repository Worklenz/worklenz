...
const EditPreviewAndSubmitStep = ({
  ...
  const handleSave = async () => {
    if (!id) {
      message.error(t('serviceIdMissing', { defaultValue: 'Service ID is missing' }));
      return;
    }
    ...
    try {
      ...
      message.error(t('serviceUpdateFailed', { defaultValue: 'Failed to update service' }));
    }
  };
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
          ...
          >
            <Flex vertical gap={16}>
              ...
              <div>
                 ...
                 >
                   {t('addService.serviceDetails.serviceDescription', { defaultValue: 'Service Description' })}:
                 </Typography.Text>
                {service?.service_data?.description ? (
                  <div
                    style={{
              ...
              title={
                <Flex align="center" gap={8}>
                   <Typography.Title level={4} style={{ margin: 0 }}>
                     {t('requestFormPreview', { defaultValue: 'Request Form Preview' })}
                   </Typography.Title>
                   <Tag color="blue">
                     {service.service_data.request_form.length} {t('questionsCount', { defaultValue: 'question(s)' })}
                  </Tag>
                </Flex>
              }
              style={{ boxShadow: `0 2px 8px ${token.colorBgTextHover}` }}
                    ...
                    >
                      <Flex vertical gap={8}>
                        <Flex justify="space-between" align="start">
                          ...
                          >
                             {t(`${item.type}Option`, { defaultValue: item.type + ' Option' })}
                          </Tag>
                        </Flex>
                          ...
                          Array.isArray(item.answer) && (
                            <div>
                              <Typography.Text
                                ...
                                style={{ fontSize: 12, display: 'block', marginBottom: 4 }}
                              >
                                 {t('optionsLabel', { defaultValue: 'Options:' })}
                              </Typography.Text>
                              <Flex wrap gap={4}>
      ...
      <div style={{ borderTop: `1px solid ${token.colorBorder}`, paddingTop: 16, flexShrink: 0 }}>
        <Space style={{ width: '100%', justifyContent: 'flex-end' }}>
          <Button onClick={() => setCurrent(1)}>{t('previousButton', { defaultValue: 'Previous' })}</Button>
          <Button type="primary" onClick={handleSave} loading={isLoading}>
            {isEdit ? t('updateButton', { defaultValue: 'Update' }) : t('submitButton', { defaultValue: 'Submit' })}
          </Button>
        </Space>
      </div>
    </div>
