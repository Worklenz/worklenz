...
export const projectMembersApiService = {
  ...
  getInvitationLinkStatus: async (projectId: string): Promise<IServerResponse<{
    ...
    const response = await apiClient.get<IServerResponse<any>>(`${rootUrl}/invitation-link/status?${params}`);
    return response.data;
  },

  revokeInvitationLink: async (projectId: string): Promise<IServerResponse<any>> => {
    const response = await apiClient.put<IServerResponse<any>>(`${rootUrl}/invitation-link/revoke`, {
      project_id: projectId
    });
    return response.data;
  },

  validateInvitationLink: async (token: string): Promise<IServerResponse<{
    project: {
      id: string;
      name: string;
      color_code: string;
    ...
    const response = await apiClient.get<IServerResponse<any>>(`${rootUrl}/invitation-link/validate/${token}`);
    return response.data;
  },

  acceptInvitationByLink: async (token: string, body: {
    name: string;
    email: string;
  }): Promise<IServerResponse<any>> => {
    const response = await apiClient.post<IServerResponse<any>>(`${rootUrl}/invitation-link/accept/${token}`, body);
    return response.data;
  },
};
